...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.act(x)
        # x = self.drop(x)
        # commit this for the original BERT implementation
        x = self.fc2(x)
        x = self.drop(x)
        return x
...
class VisionTransformer(nn.Module):
    ...
    def forward_tokens(self, x):
        """
        Return final patch tokens before mean pooling / classification head.
        Output shape: (B, N, D), where N is the number of image patches.
        """
